# This makefile implements wrappers around various tofu/terraform test commands.
...
TF_SETUP_SENTINEL := tests/setup/harness.yml
...
test.%: $(TF_SETUP_SENTINEL)
	$(TF_EXECUTABLE) init
	$(TF_EXECUTABLE) test --filter=tests/$*.tofutest.hcl

EXAMPLES :=

$(TF_SETUP_SENTINEL): $(wildcard tests/setup/*.tf) $(filter-out $(TF_SETUP_SENTINEL), $(wildcard tests/setup/*.tfvars))
...
tests/ephemeral/%/main.tf: $(wildcard examples/%/*.tf)
	mkdir -p $(@D)
	rsync -avP --exclude .terraform \
		--exclude .terraform.lock.hcl \
		--exclude 'terraform.tfstate' \
		examples/$*/ $(@D)/
	sed -i '' -E -e '1h;2,$$H;$$!d;g' -e 's@module "site"[ \t]*{[ \t]*\n[ \t]*source[ \t]*=[ \t]*"[^"]+"@module "site" {\n  source = "../../../"@' $@
...
pre-release.%:
	...
	@find examples -type f -name main.tf -print0 | \
		xargs -0 awk 'BEGIN{m=0;s=0;v=0}; /module "site"/ {m=1}; m==1 && /source[ \t]*=[ \t]*"memes\/volterra\/google/ {s++}; m==1 && /version[ \t]*=[ \t]*"$(subst .,\.,$(*:v%=%))"/ {v++}; END{if (s==0) { printf "%s has incorrect source\n", FILENAME}; if (v==0) { printf "%s has incorrect version\n", FILENAME}; if (s==0 || v==0) { exit 1}}'
	@find examples -type f -name main.tf -print0 | \
	...
	@grep -Eq '^version:[ \t]*$(subst .,\.,$(*:v%=%))[ \t]*$$' tests/profiles/access/inspec.yml || \
		(echo "tests/profiles/access/inspec.yml has incorrect tag"; exit 1)
	@grep -Eq '^version:[ \t]*$(subst .,\.,$(*:v%=%))[ \t]*$$' tests/profiles/cluster/inspec.yml || \
		(echo "tests/profiles/cluster/inspec.yml has incorrect tag"; exit 1)
	@grep -Eq '^version:[ \t]*$(subst .,\.,$(*:v%=%))[ \t]*$$' tests/profiles/sa/inspec.yml || \
		(echo "tests/profiles/sa/inspec.yml has incorrect tag"; exit 1)
